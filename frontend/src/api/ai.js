...
export const aiAPI = {
  ...
  getChecklist: (country) =>
    apiClient.get(`/ai/checklist/${encodeURIComponent(country)}`).then(r => r.data),

  saveChecklist: (country, checklist, checked, metadata = {}) =>
    apiClient.put(`/ai/checklist/${encodeURIComponent(country)}`, { checklist, checked, metadata }).then(r => r.data),
  ...
  generateSop: (profile = {}, university = '', program = '', country = '') =>
    apiClient.post('/ai/generate-sop', { profile, university, program, country }).then(r => r.data),
};
